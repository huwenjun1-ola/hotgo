...
type (
	...
	ISysGameDiffLevelConfig interface {
		// Model 找茬关卡配置表ORM模型
		Model(ctx context.Context, option ...*handler.Option) *gdb.Model
		// List 获取找茬关卡配置表列表
		List(ctx context.Context, in *sysin.GameDiffLevelConfigListInp) (list []*sysin.GameDiffLevelConfigListModel, totalCount int, err error)
		// Export 导出找茬关卡配置表
		Export(ctx context.Context, in *sysin.GameDiffLevelConfigListInp) (err error)
		// Edit 修改/新增找茬关卡配置表
		Edit(ctx context.Context, in *sysin.GameDiffLevelConfigEditInp) (err error)
		// Delete 删除找茬关卡配置表
		Delete(ctx context.Context, in *sysin.GameDiffLevelConfigDeleteInp) (err error)
		// View 获取找茬关卡配置表指定信息
		View(ctx context.Context, in *sysin.GameDiffLevelConfigViewInp) (res *sysin.GameDiffLevelConfigViewModel, err error)
	}
	ISysGameRoute interface {
		// Model 游戏路由ORM模型
		Model(ctx context.Context, option ...*handler.Option) *gdb.Model
		// List 获取游戏路由列表
		List(ctx context.Context, in *sysin.GameRouteListInp) (list []*sysin.GameRouteListModel, totalCount int, err error)
		// Export 导出游戏路由
		Export(ctx context.Context, in *sysin.GameRouteListInp) (err error)
		// Edit 修改/新增游戏路由
		Edit(ctx context.Context, in *sysin.GameRouteEditInp) (err error)
		// Delete 删除游戏路由
		Delete(ctx context.Context, in *sysin.GameRouteDeleteInp) (err error)
		// View 获取游戏路由指定信息
		View(ctx context.Context, in *sysin.GameRouteViewInp) (res *sysin.GameRouteViewModel, err error)
		// List 获取游戏路由列表
		GetGameRouteByFactory(ctx context.Context, factoryType string) (model *sysin.GameRouteListModel, err error)
	}
...
var (
	...
	localSysGameDiffLevelConfig ISysGameDiffLevelConfig
	localSysGameRoute           ISysGameRoute
...
func SysGameRoute() ISysGameRoute {
	if localSysGameRoute == nil {
		panic("implement not found for interface ISysGameRoute, forgot register?")
	}
	return localSysGameRoute
}

func RegisterSysGameRoute(i ISysGameRoute) {
	localSysGameRoute = i
}
...
func SysGameDiffLevelConfig() ISysGameDiffLevelConfig {
	if localSysGameDiffLevelConfig == nil {
		panic("implement not found for interface ISysGameDiffLevelConfig, forgot register?")
	}
	return localSysGameDiffLevelConfig
}

func RegisterSysGameDiffLevelConfig(i ISysGameDiffLevelConfig) {
	localSysGameDiffLevelConfig = i
}
